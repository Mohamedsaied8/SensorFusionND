...
template <typename PointT>
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr>
ProcessPointClouds<PointT>::Ransac(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations,
    ...
    while (maxIterations--) {
        ...
        std::unordered_set<int> inliers; // temp map
...
template <typename PointT>
void ProcessPointClouds<PointT>::pointCollect(const std::vector<std::vector<float>> &points,
    ...
    for (int nearbyIndex : nearbyPoints) {
        if (!processed.at(nearbyIndex)) {
            pointCollect(points, cluster, processed, nearbyIndex, tolerance, tree);
        }
    }
}
...
ProcessPointClouds<PointT>::euclideanClustering(const std::vector<std::vector<float>> &points, KdTree *tree,
                                             float distanceTol) {
    ...
    for (int i{ 0 }; i < points.size(); i++) {
        if (!processed.at(i)) {
            ...
            pointCollect(points, cluster, processed, i, distanceTol, tree);
...
ProcessPointClouds<PointT>::Clustering(typename pcl::PointCloud<PointT>::Ptr cloud,
    ...
    const std::vector<std::vector<int>> listOfIndices{ euclideanClustering(points, kdTree, clusterTolerance) };
