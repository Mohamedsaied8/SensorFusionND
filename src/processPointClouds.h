...
template <typename PointT> class ProcessPointClouds {
  public:
    ...
    void pointCollect(const std::vector<std::vector<float>> &points, std::vector<int> &cluster,
                   std::vector<bool> &processed, const int index, const float tolerance, KdTree *tree);

    std::vector<std::vector<int>> euclideanClustering(const std::vector<std::vector<float>> &points,
